...
class ColorBlobLocatorProcessorImpl extends ColorBlobLocatorProcessor implements VisionProcessor
{
    ...
    public Object processFrame(Mat frame, long captureTimeNanos)
    {
        ...
        if (colorRange.max.val[0] < colorRange.min.val[0]){
            Scalar upperMidScalar = new Scalar(colorRange.min.val[0], colorRange.min.val[1], colorRange.min.val[2]);
            Scalar upperHighScalar = new Scalar(256, colorRange.max.val[1], colorRange.max.val[2]);

            Scalar lowerLowScalar = new Scalar(-20, colorRange.min.val[1], colorRange.min.val[2]);
            Scalar lowerMidScalar = new Scalar(colorRange.max.val[0], colorRange.max.val[1], colorRange.max.val[2]);

            ColorRange upperRange = new ColorRange(ColorSpace.HSV, upperMidScalar, upperHighScalar);
            ColorRange lowerRange = new ColorRange(ColorSpace.HSV, lowerLowScalar, lowerMidScalar);
            Mat upperMask = new Mat();
            Mat lowerMask = new Mat();
            Mat kernel = Imgproc.getStructuringElement(Imgproc.MORPH_ELLIPSE, new Size(5, 5));
            Core.inRange(roiMat_userColorSpace, upperRange.min, upperRange.max, upperMask);
            Imgproc.dilate(upperMask, upperMask, kernel, new Point(-1, -1), 2);
            Imgproc.morphologyEx(upperMask, upperMask, Imgproc.MORPH_CLOSE, kernel);
            Core.inRange(roiMat_userColorSpace, lowerRange.min, lowerRange.max, lowerMask);
            Imgproc.dilate(lowerMask, lowerMask, kernel, new Point(-1, -1), 2);
            Imgproc.morphologyEx(lowerMask, lowerMask, Imgproc.MORPH_CLOSE, kernel);
            Core.bitwise_or(upperMask, lowerMask, mask);
            upperMask.release();
            lowerMask.release();
        } else {
