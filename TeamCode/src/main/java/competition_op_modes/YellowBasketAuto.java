...
public class YellowBasketAuto extends OpMode {
    private enum PathStates{
        START(new Pose(10, 52, 0)),
        APPROACH_SCORE(new Pose(22.5, 66.5, -PI / 4.0)),
        SCORE(new Pose(APPROACH_SCORE.finalPose.getX() - 5, APPROACH_SCORE.finalPose.getY() + 4, -PI / 4.0)),
        RELEASING(null),
        PICKUP_FIRST(new Pose(32.5, 60.2, 0)),
        PICKUP_SECOND(new Pose(32.9, 70, 0)),
        PICKUP_THIRD(new Pose(48, 69, PI / 2.0)),
        GRABBING(null),
        ...
        WAIT_TO_STABILIZE(null),
        SUB_WAIT_SAMPLE_GRAB(null),
        ;
        Pose finalPose;
        PathStates(com.pedropathing.localization.Pose finalPose){
    ...
    void buildPaths(){
        scorePreloadPath = follower.pathBuilder().addPath(new Path(new BezierCurve(
                ...
                new Point(PathStates.APPROACH_SCORE.finalPose)
        ))).build();
        scorePreloadPath.getPath(0).setLinearHeadingInterpolation(PathStates.START.finalPose.getHeading(), PathStates.APPROACH_SCORE.finalPose.getHeading());
        for (int pathIndex = 0; pathIndex < scorePaths.length; pathIndex++){
            scorePaths[pathIndex] = follower.pathBuilder().addPath(new Path(new BezierCurve(
                    new Point(PathStates.APPROACH_SCORE.finalPose),
                    new Point(PathStates.SCORE.finalPose)
            ))).build();
            scorePaths[pathIndex].getPath(0).setConstantHeadingInterpolation(PathStates.SCORE.finalPose.getHeading());
        }
        grabPaths[0] = follower.pathBuilder().addPath(new Path(new BezierCurve(
                new Point(PathStates.SCORE.finalPose),
                new Point(PathStates.SCORE.finalPose.getX() + 4, PathStates.SCORE.finalPose.getY() - 4),
...
//        grabPaths[0].setLinearHeadingInterpolation(PathStates.SCORE.finalPose.getHeading(), PathStates.PICKUP_FIRST.finalPose.getHeading());
...
//        grabPaths[1].setLinearHeadingInterpolation(PathStates.SCORE.finalPose.getHeading(), PathStates.PICKUP_SECOND.finalPose.getHeading());
...
//        grabPaths[2].setLinearHeadingInterpolation(PathStates.SCORE.finalPose.getHeading(), PathStates.PICKUP_THIRD.finalPose.getHeading());
        ...
        aimPaths[0] = follower.pathBuilder().addPath(new Path(new BezierCurve(
                ...
                new Point(PathStates.APPROACH_SCORE.finalPose.getX() + 5, PathStates.APPROACH_SCORE.finalPose.getY() - 5),
                new Point(PathStates.APPROACH_SCORE.finalPose)
        ))).build();
...
//        aimPaths[0].setLinearHeadingInterpolation(PathStates.PICKUP_FIRST.finalPose.getHeading(), PathStates.APPROACH_SCORE.finalPose.getHeading());
...
//                new Point(PathStates.APPROACH_SCORE.finalPose.getX() + 10, PathStates.APPROACH_SCORE.finalPose.getY() - 3),
                new Point(PathStates.APPROACH_SCORE.finalPose.getX() + 3, PathStates.APPROACH_SCORE.finalPose.getY() - 3),
                new Point(PathStates.APPROACH_SCORE.finalPose)
        ))).build();
...
//        aimPaths[1].setLinearHeadingInterpolation(PathStates.PICKUP_SECOND.finalPose.getHeading(), PathStates.APPROACH_SCORE.finalPose.getHeading());
        ...
        parkPath.getPath(0).setLinearHeadingInterpolation(PathStates.SCORE.finalPose.getHeading(), PathStates.PARK.finalPose.getHeading());
    }
...
//        else if (liftController.getCurrentPosition() > 600)
    ...
    void updatePath(){
        switch (pathState){
            case START:
                setPathState(PathStates.APPROACH_SCORE);
                ...
                break;
            case APPROACH_SCORE:
                if (scoreIteration >= 3 && actionTimer.milliseconds() > 300 && stateMachineController.getActionState() == StateMachineController.ActionStates.FREE && depositController.getCurrentState()== DepositController.DepositArmStates.TRANSFER_READY)
                    stateMachineController.transfer();
                if (stateMachineController.getActionState() == StateMachineController.ActionStates.FREE && (depositController.getCurrentState() == DepositController.DepositArmStates.HUMAN_DROP || depositController.getCurrentState() == DepositController.DepositArmStates.BASKET) && follower.atPoint(new Point(PathStates.APPROACH_SCORE.finalPose), 40, 40))
                    LiftController.setFinalTargetPosition(LiftController.UPPER_BASKET_HEIGHT);
                if (follower.atPoint(new Point(PathStates.APPROACH_SCORE.finalPose), 4, 4) && liftController.getCurrentPosition() > 1170 && stateMachineController.getActionState() == StateMachineController.ActionStates.FREE){
                    setPathState(PathStates.SCORE);
                    follower.followPath(scorePaths[scoreIteration], true);
                }
                break;
            case SCORE:
                if (stateMachineController.getActionState() == StateMachineController.ActionStates.FREE && (depositController.getCurrentState() == DepositController.DepositArmStates.HUMAN_DROP || depositController.getCurrentState() == DepositController.DepositArmStates.BASKET))
                    LiftController.setFinalTargetPosition(LiftController.UPPER_BASKET_HEIGHT);
                if (follower.atPoint(new Point(PathStates.SCORE.finalPose), 2, 2)){
            ...
            case PICKUP_FIRST:
                ...
                if (follower.atPoint(new Point(PathStates.PICKUP_FIRST.finalPose), 12, 12) && ExtendoController.getFinalTargetPosition() <= 0){
                    ExtendoController.setFinalTargetPosition(160, true);
                }
...
//                if (follower.atPoint(new Point(PathStates.PICKUP_FIRST.finalPose), 1, 1)){
                if (actionTimer.milliseconds() > 6000 || (follower.atPoint(new Point(PathStates.PICKUP_FIRST.finalPose), 1, 0.5) && follower.getVelocityMagnitude() < 7 && !extendoController.isBusy())){
            ...
            case PICKUP_SECOND:
                if (!follower.atPoint(new Point(PathStates.SCORE.finalPose), 4, 4))
                    LiftController.setFinalTargetPosition(LiftController.bottomLimit);
                if (follower.atPoint(new Point(PathStates.PICKUP_SECOND.finalPose), 12, 12) && ExtendoController.getFinalTargetPosition() <= 0){
                    ExtendoController.setFinalTargetPosition(160, true);
                }
                if (follower.atPoint(new Point(PathStates.PICKUP_SECOND.finalPose), 6, 6)){
                    follower.setMaxPower(0.65);
                }
                if (actionTimer.milliseconds() > 5000 || (follower.atPoint(new Point(PathStates.PICKUP_SECOND.finalPose), 1, 0.5) && follower.getVelocityMagnitude() < 14)){
//                if (follower.atPoint(new Point(PathStates.PICKUP_SECOND.finalPose), 1, 1)){
            ...
            case PICKUP_THIRD:
                ...
                if (follower.atPoint(new Point(PathStates.PICKUP_THIRD.finalPose), 6, 6))
                    follower.setMaxPower(0.65);
                if (actionTimer.milliseconds() > 5000 || (follower.atPose(PathStates.PICKUP_THIRD.finalPose, 1, 0.5, PI / 30.0) && follower.getVelocityMagnitude() < 14)){
...
//                    if (llController.noDetections() && follower.getPose().getX() < 90){
//                        setPathState(PathStates.SHIFT_SUBMERSIBLE);
//                        shiftSubmersiblePose = new Pose(follower.getPose().getX() + 15, follower.getPose().getY(), PathStates.APPROACH_SUBMERSIBLE.finalPose.getHeading());
//                        shiftSubmersiblePath = follower.pathBuilder().addPath(new Path(new BezierCurve(
//                                new Point(follower.getPose()),
//                                new Point(follower.getPose().getX(), follower.getPose().getY() + 10),
//                                new Point(follower.getPose().getX() + 15, follower.getPose().getY() + 10),
//                                new Point(follower.getPose().getX() + 15, follower.getPose().getY())
//                        ))).build();
//                        shiftSubmersiblePath.getPath(0).setConstantHeadingInterpolation(PathStates.APPROACH_SUBMERSIBLE.finalPose.getHeading());
//                        follower.followPath(shiftSubmersiblePath.getPath(0));
//                        break;
//                    }
...
//                    LiftController.setFinalTargetPosition(200);
            ...
            case GO_TO_DETECTION:
                ...
                if ((follower.atPose(detectionPose, 1, 1, PI / 36.0)) && follower.getVelocityMagnitude() < 9 && !extendoController.isBusy()){
//                if (!follower.isBusy() && !extendoController.isBusy()){
                    setPathState(PathStates.WAIT_TO_STABILIZE);
                }
                break;
            case WAIT_TO_STABILIZE:
                if (actionTimer.milliseconds() > 150){
                    setPathState(PathStates.SUB_WAIT_SAMPLE_GRAB);
                    stateMachineController.grab();
                }
                break;
            case SUB_WAIT_SAMPLE_GRAB:
                if (actionTimer.milliseconds() > 500){
                    Path fromSubApproachPath = new Path(new BezierCurve(
                            ...
                            new Point(PathStates.APPROACH_SCORE.finalPose.getX() + 4, PathStates.APPROACH_SCORE.finalPose.getY() - 4),
                            new Point(PathStates.APPROACH_SCORE.finalPose.getX() + 16, PathStates.APPROACH_SCORE.finalPose.getY() - 4),
                            new Point(PathStates.APPROACH_SCORE.finalPose)
                    ));
